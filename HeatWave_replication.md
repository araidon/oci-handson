...
今回、VPN接続の設定方法は以下記事を参考にしてください。

[VPN接続の設定方法](https://qiita.com/yama6/items/c188de191269cb604341)
...
1. カスタムパラメータグループ にて以下3オプションの有効化を実施	
- `GTID-MODE=ON`
...
2. 構築時のオプションにて①で作成したカスタムパラメータグループの指定
...
2. レプリケーションユーザ作成
以下のパスワードポリシーに準拠した任意のパスワードを設定してください。  
- パスワードは8文字から32文字までの長さで、大文字、小文字、数字および特殊文字をそれぞれ1つ以上含む必要があります。  
...
## 5-2. HeatWave MySQLにてレプリケーション確認

1. HeatWave MySQLにアクセス
```
